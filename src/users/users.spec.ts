...
// Firma real de manager.update: (entityClass, criteria, partialEntity). Se
// tipa como unknown/Record<string, unknown> (no jest.Mocked<EntityManager>)
// porque los tests invocan tanto con User como con Task como entityClass.
type ManagerUpdateCall = [
  entityClass: unknown,
  criteria: unknown,
...
const mockManager = {
  ...
  update: jest.fn<Promise<unknown>, ManagerUpdateCall>(),
...
describe('UsersService', () => {
  ...
  describe('deactivate', () => {
    ...
    it('lanza NotFoundException (404) si el id no existe', async () => {
      ...
      await expect(
        service.deactivate('nonexistent', adminActor),
    ...
    it('lanza ForbiddenException (403) si el target tiene role ADMIN', async () => {
      ...
      await expect(
        service.deactivate('other-admin', adminActor),
    ...
    it('desactiva el usuario y devuelve UserResponseDto con isActive false', async () => {
      ...
      expect(mockManager.update).toHaveBeenCalledWith(User, 'uuid-1', {
        isActive: false,
      });
    ...
    it('trampa del join destructivo: un developer co-asignado en el mismo proyecto sobrevive a la cascada', async () => {
      ...
      const savedProjects = mockManager.save.mock.calls[0][0] as {
        developers: User[];
      }[];
    ...
    it('cascada de tareas: desactivar un developer con proyectos limpia el assigneeId de sus tareas no-done, atómicamente', async () => {
      ...
      mockManager.find.mockResolvedValue([
        { id: 'task-1', projectId: 'proj-1', assigneeId: 'uuid-1' },
      ]);
    ...
    it('desactivar un developer con proyectos escribe una fila de auditoría actorKind system por tarea desasignada', async () => {
      const target = makeUser({ id: 'uuid-1', isActive: true });
      const project = makeProject({ id: 'proj-1', developers: [target] });
      const reloaded = makeUser({ id: 'uuid-1', isActive: false });
      mockRepository.findOne
  ...
  describe('update', () => {
    it('lanza NotFoundException (404) si el id no existe', async () => {
      ...
      await expect(
        service.update('nonexistent', { role: UserRole.ADMIN }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('password provisto: se hashea con bcrypt y refreshToken se pone en null', async () => {
      ...
      const [, , changes] = mockManager.update.mock.calls[0] as [
        unknown,
        unknown,
        Partial<User>,
      ];
    ...
    it('demover a un developer con solo tareas done preserva el assigneeId (cascada no-op, la escritura Task nunca se llama)', async () => {
      ...
      mockQueryBuilder.getMany.mockResolvedValue([project]);
      mockManager.save.mockResolvedValue([project]);
      // Todas las tareas del developer están done: el find (que ya excluye
      // DONE) no devuelve filas -> early return, sin update ni insert.
      mockManager.find.mockResolvedValue([]);

      await service.update('uuid-1', { role: UserRole.ADMIN });

      const taskUpdateCall = mockManager.update.mock.calls.find(
        (call) => call[0] === Task,
      );
  ...
  describe('reactivate', () => {
    ...
    it('lanza BadRequestException (400) si el usuario ya está activo', async () => {
      ...
      await expect(service.reactivate('uuid-1')).rejects.toThrow(
        BadRequestException,
      );
    });
